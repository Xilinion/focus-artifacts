...
static inline uint64_t
trace_ats_inc(uint64_t key_hash)
{
   uint64_t idx = key_hash & TRACE_ATS_MASK;
   return __atomic_fetch_add(&g_trace_ats[idx], 1, __ATOMIC_RELAXED) + 1;
}

static inline uint64_t
trace_ats_read(uint64_t key_hash)
{
   uint64_t idx = key_hash & TRACE_ATS_MASK;
...
RETRY_LOCK_WRITE_SET:
{
   ...
   if (!is_abort) {
      ...
      for (uint64 i = 0; i < num_writes; ++i) {
         ...
         if (has_bufs
             && g_trace_state[tid].write_count < TRACE_BUF_SIZE)
         {
            uint64        key_hash = trace_fnv64(slice_data(w->key),
                                          slice_length(w->key));
            ...
            wr->new_wts      = (uint64_t)commit_ts; // commit_ts IS the new wts
      ...
      if (tid < MAX_THREADS && g_trace_state[tid].abort_buf != NULL) {
         ...
         for (uint64 i = 0; i < num_reads; ++i) {
                ...
                && g_trace_state[tid].abort_count < TRACE_BUF_SIZE)
            {
               ...
               abort_record *ar      = &g_trace_state[tid]
                                          .abort_buf[g_trace_state[tid]
                                                        .abort_count++];
               ar->txn_seq      = my_seq;
